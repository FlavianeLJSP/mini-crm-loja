...
let clientesCache = {};
let produtosCache = {};
let produtosCompletosCache = {}; // NOVO: Cache para objetos de produto completos
...
function mostrarSecao(targetId) {
  ...
  topHeader.querySelector("h2").textContent = document
...
function abrirModalCadastro(tipo) {
  ...
  switch (tipo) {
    ...
    case "venda":
      ...
      setTimeout(() => {
        const modalClienteVenda = document.getElementById(
          "cliente-venda-modal"
        );
...
modalCadastroSalvarBtn.addEventListener("click", async () => {
  let colecao =
    tipoCadastroAtual +
    (tipoCadastroAtual === "cliente" ||
    tipoCadastroAtual === "produto" ||
    tipoCadastroAtual === "insumo"
      ? "s"
      : "s");
  ...
  if (tipoCadastroAtual === "cliente") {
    ...
    novoItem = {
      ...
      formaPagamentoCompra: formElement.querySelector("#forma-pagamento-compra")
        .value,
    };
    if (!novoItem.nome || isNaN(novoItem.valorPago)) {
      alert("Nome e Valor Pago são obrigatórios.");
      return;
    }
    ...
    tipoCadastroAtual === "recebimento" ||
    tipoCadastroAtual === "despesa"
  ) {
    isFluxo = true;
    novoItem = {
      ...
      origem:
        tipoCadastroAtual === "recebimento"
          ? formElement.querySelector("#origem").value
          : "",
      destino:
        tipoCadastroAtual === "despesa"
          ? formElement.querySelector("#destino").value
          : "",
      data: formElement.querySelector(
        tipoCadastroAtual === "recebimento"
          ? "#data-recebimento"
          : "#data-despesa"
      ).value,
      descricao: formElement.querySelector(
        tipoCadastroAtual === "recebimento"
    ...
    if (isNaN(novoItem.valor) || (!novoItem.origem && !novoItem.destino)) {
      alert("Valor e origem/destino são obrigatórios.");
      return;
    }
    ...
    novoItem = {
      ...
      quantidade: parseFloat(
        formElement.querySelector("#quantidade-insumo").value
      ),
    ...
    if (!novoItem.nome || isNaN(novoItem.custo)) {
      alert("Nome e Custo são obrigatórios para Insumo.");
      return;
    }
  } else {
  ...
  clientes.forEach((cliente) => {
    ...
    tr.innerHTML = `
            ...
            <td>
                ...
                <button class="tabela-acao-btn delete-btn" onclick="deletarCliente('${
                  cliente.id
                }')">Excluir</button>
            </td>
        `;
...
function abrirModalEdicaoCliente(clienteData) {
  ...
  edicaoCampos.innerHTML = `
        ...
        <div>
            ...
            <input type="text" id="edicao-telefone" value="${
              clienteData.telefone
            }" required>
        </div>
        <div>
            <label for="edicao-endereco">Endereço:</label>
            <input type="text" id="edicao-endereco" value="${
              clienteData.endereco
            }">
        </div>
        <div>
            <label for="edicao-statusPagamento">Status Pagamento (Texto Livre):</label>
            <input type="text" id="edicao-statusPagamento" value="${
              clienteData.statusPagamento || ""
            }">
        </div>
    `;
...
function renderizarProdutos(produtos) {
  ...
  selectProdutoVenda.innerHTML =
    '<option value="">Selecione um Produto</option>';

  // ATUALIZA O CACHE DE PRODUTOS
  produtosCache = {};
  produtosCompletosCache = {}; // Limpa o novo cache
  ...
  produtos.forEach((produto) => {
    ...
    produtosCompletosCache[produto.id] = produto; // Popula o novo cache com o objeto completo
    ...
    tr.innerHTML = `
            ...
            <td>R$ ${
              produto.valorPago ? produto.valorPago.toFixed(2) : "0.00"
            }</td>
            ...
            <td>
                ...
                <button class="tabela-acao-btn delete-btn" onclick="deletarProduto('${
                  produto.id
                }')">Excluir</button>
            </td>
        `;
    ...
    option.textContent = `${produto.nome} (Custo: R$ ${
      produto.valorPago ? produto.valorPago.toFixed(2) : "0.00"
    })`;
...
function abrirModalEdicaoProduto(produtoData) {
  ...
  const vencimento =
    produtoData.vencimento === "N/A" ? "" : produtoData.vencimento;
  const dataCompra =
    produtoData.dataCompra === "N/A" ? "" : produtoData.dataCompra;
  ...
  edicaoCampos.innerHTML = `
        ...
        <div>
            ...
            <input type="text" id="edicao-codigo" value="${
              produtoData.codigo || ""
            }">
        </div>
        <div>
            <label for="edicao-estoque">Estoque:</label>
            <input type="number" id="edicao-estoque" step="1" value="${
              produtoData.estoque
            }" required>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="number" id="edicao-valorPago" step="0.01" value="${
              produtoData.valorPago
            }" required>
        </div>
        <div>
            <label for="edicao-fornecedor">Fornecedor:</label>
            <input type="text" id="edicao-fornecedor" value="${
        ...
        <div>
            ...
            <input type="text" id="edicao-formaPagamentoCompra" value="${
              produtoData.formaPagamentoCompra || ""
            }">
        </div>
    `;
...
function renderizarVendas(vendas) {
  ...
  vendas.forEach((venda) => {
    ...
    const nomeCliente =
      clientesCache[venda.clienteId] ||
      `ID: ${venda.clienteId.substring(0, 5)}...`;
    ...
    const produtosHtml =
      venda.produtos && venda.produtos.length > 0
        ? venda.produtos
            .map((p) => {
              const nomeProduto =
                produtosCache[p.produtoId] ||
                `ID: ${p.produtoId.substring(0, 5)}...`;
              return `<li>${
                p.quantidade
              }x ${nomeProduto} (R$ ${p.precoUnitario.toFixed(2)})</li>`;
            })
            .join("")
    ...
    tr.innerHTML = `
            ...
            <td>${
              venda.produtos
                ? venda.produtos.reduce((acc, p) => acc + p.quantidade, 0)
                : 0
            }</td>
            <td>R$ ${
            ...
            <td>
                <button class="tabela-acao-btn delete-btn" onclick="deletarVenda('${
                  venda.id
                }')">Excluir</button>
            </td>
        `;
...
function adicionarLinhaProdutoVenda() {
  ...
  for (const id in produtosCache) {
    ...
    const produtoOriginal = produtosCompletosCache[id]; // CORREÇÃO: Usa o novo cache
    const custo =
      produtoOriginal && produtoOriginal.valorPago
        ? produtoOriginal.valorPago.toFixed(2)
        : "0.00";
    opcoesProdutoHTML += `<option value="${id}">${produtoOriginal.nome} (Custo: R$ ${custo})</option>`;
  }
...
function calcularTotalVenda() {
  ...
  produtoRows.forEach((row) => {
    const quantidade =
      parseFloat(row.querySelector(".quantidade-input").value) || 0;
...
function renderizarFluxo(itens) {
  if (!listaCaixaBody) {
    console.warn(
      "Elemento 'lista-caixa-body' não encontrado. Fluxo de Caixa não será exibido."
    );
    return;
  }
